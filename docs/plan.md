...
3. Parse response tolerantly, handling BOTH known shapes:
   ...
   - Newer `limits[]` array (preferred when present): live kinds observed 2026-07-13 map to stable keys shared with the top-level shape - `session` -> `five_hour` / "5-hour limit"; `weekly_all` (or `seven_day` without model) -> `seven_day` / "Weekly - all models"; `weekly_scoped` / `seven_day` with `scope.model.display_name` -> `seven_day_<slug>` / "Weekly - <display_name>". Live entries often have `"scope": null` (not omitted) - zod must use `.nullish()`. Older fixtures may still use `kind: five_hour|seven_day`. Validate each limits entry individually so one odd entry never kills the fetch. When both top-level buckets and `limits[]` are present, prefer `limits[]`.
